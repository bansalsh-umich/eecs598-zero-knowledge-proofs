...
//     let mut commitments = Vec::with_capacity(statement.num_vars);
//     let mut g_prev = witness.polynomial.clone();

//     let chunk_size = statement.max_degree + 1;
//     let generator_chunks = params.vec_gens.chunks_exact(chunk_size);

//     let mut o_sum = E::Scalar::zero();

//     // Step 1: Compute the Commitments
//     for (i, generators) in generator_chunks.enumerate() {
//         let g_i = g_prev.to_univariate(0);
//         let o_i = E::Scalar::random(&mut rng);
//         let commitment = pedersen::vector_commit(&g_i.coeffs()[..chunk_size], o_i, generators, h);
//         o_sum += o_i;

//         trans.append_message(format!("C{i}").as_str(), commitment);
//         commitments.push(commitment);

//         let challenge: E::Scalar = trans.get_challenge(format!("challenge_{i}").as_str());
//         g_prev = g_prev.partial_eval(std::slice::from_ref(&challenge));
//         challenges.push(challenge);
//     }

//     // Step 2: Final Round
//     let final_eval = g_prev.evaluate(&[]);
//     let o_final = E::Scalar::random(&mut rng);
//     let final_commitment = pedersen::commit(final_eval, o_final, &params.scalar_gens);
//     trans.append_message("comm_final", final_commitment);
...
//     let comm_sum: E = commitments.iter().copied().sum();
//     let sumcheck_result = E::msm(
//         &[rho[0], rho[statement.num_vars]],
//         &[statement.comm_sum, final_commitment],
//     );
...
//     pedersen::dot_product::prove(&dp_params, &dp_statement, , trans, rng)
